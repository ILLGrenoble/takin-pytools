...
def calc(param):
    ...
    covInstr = covInstrument(VarPx, VarPy, VarPz, VarMx, VarMy, VarMz, VarSx, VarSy, VarSz, VarDx, VarDy, VarDz, Vartp, Vartm, Vartd, CovDxDy)

    LPM, LPMx, LPMy, LPMz, LMS, LMSx, LMSy, LMSz = length(Sr, Sh, Lpe, Lme, Les, Eyh, Ezh, -(Lpe+Les), np.sqrt(VarPx), -(Lme+Les), np.sqrt(VarMx), thetacrit)
    ...
    E = tas.get_E(ki, kf)
    covQhw = cov(vi, vf, dict_L, covInstr, det_shape, verbose)
...
print(k2v(2*np.pi/6)*1e-7)
print(np.divide( 9 - 8.5, 6*14000 ))
print(np.divide( 0.25, 6*14000 )*k2v(2*np.pi/2)*1e-7)
