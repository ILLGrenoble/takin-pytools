...
def calc_dispersion(is_ferromagnetic = True):
	...
	plt.xlabel("h (rlu)")
	plt.ylabel("E (meV)")
	plt.scatter(hs, Es, marker = '.', s = ws, label = title)
	return (hs, Es, ws), (sites, couplings)

print("Calculating ferromagnetic dispersion...")
ferro_data = calc_dispersion(True)

print("Calculating antiferromagnetic dispersion...")
anti_ferro_data = calc_dispersion(False)
